/*
  Sketch for SI4733-A10 / Si4735-G60 based receiver with Morse Code Readout 
  I dedicate this sketch to all blind radio amateurs.
...
// Morse defined constants  
#define BUZZY_PIN 13       // Buzzy pin
#define CW_TONE   900     // CW tone (audio frequency)
...
#define CW_READOUT_BUTTON 7     // **** Use thi button to implement a new function
...
void setup()
{
  ...
  pinMode(CW_READOUT_BUTTON, INPUT_PULLUP);
...
{
    char tmp[10];
    convertToChar(currentFrequency, tmp, 5, 0);
    playMorseCode(tmp);
    delay(200);
    playMorseCode((char *)bandModeDesc[currentMode]);
...
void loop()
{
  ...
  if ((millis() - elapsedButton) > MIN_ELAPSED_TIME) // Is that necessary? 
  {
    ...
    else if (digitalRead(CW_READOUT_BUTTON) == LOW)
    {
      playReceiverStatus();
      // showStatus();
    }
